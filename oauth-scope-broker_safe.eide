#! OAuth scope broker — untrusted a request can only ever become one of a fixed set of decisions over a
...
#! @requires issueScope — the oauth scope broker sink
#! @effect io
#! @taint bridge — extract<Decision> turns the tainted input into a trusted decision
grant issueScope

type Scope = ReadOnly | Write | Admin
type Decision = GrantScope(Scope) | Deny
...
using issueScope { privileged { issueScope(d) } }  # act on the trusted decision only
