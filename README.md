...
- **Closed type:** `type Decision = GrantScope(Scope) | Deny` where `Scope` is closed
- **Sink / capability:** `grant issueScope` → `using issueScope { privileged { issueScope(d) } }`
- **Consequence axes:** Trust, Capabilities

## Run the demo

```sh
examples/apps/oauth-scope-broker/demo.sh
```
...
- `oauth-scope-broker_safe.wave` — the correct design.
- `oauth-scope-broker_unsafe.wave` — the tempting-but-wrong version (the negative example a model must learn to reject).
